CREATE OR REPLACE FUNCTION public.get_event_guest_list(_event_id uuid)
 RETURNS TABLE(user_id uuid, display_name text, email text, is_anonymous boolean, is_co_organizer boolean, guest_count integer, joined_at timestamp with time zone, note text, payment_status boolean, payment_claimed_at timestamp with time zone, payment_date timestamp with time zone, payment_marked_by uuid, payment_marked_by_name text, receipt_url text)
 LANGUAGE plpgsql
